...
export type TripTravelSettingsUpdate = Omit<TripTravelSettings, "timezone">;
...
export function resolveDayStartMinutesFromArrival(
  ...
  let minutes = getTripClockMinutes(readyTime, timezone) ?? DEFAULT_DAY_START_MINUTES;
...
export function formatTripTimeFromIso(
  iso: string | null | undefined,
  timezone: string | null | undefined,
): string | null {
  if (!iso?.trim()) {
...
export function formatDayStartSourceLabel(
  ...
  const minutes = resolveDayStartMinutesFromArrival(flightArrival, timezone);
...
export function getDayTravelReminder(
  ...
  if (dayNumber === 1 && settings.flight_arrival) {
    const time = formatTripTimeFromIso(settings.flight_arrival, settings.timezone);
  ...
  if (dayNumber === TRIP_DAY_COUNT && settings.flight_departure) {
    const time = formatTripTimeFromIso(settings.flight_departure, settings.timezone);
...
export const TRIP_TRAVEL_SELECT =
  "timezone, start_date, flight_arrival, flight_departure, flight_outbound_number, flight_return_number, hotel_checkin, hotel_checkout, base_location, airport_transfer_minutes";

export function normalizeTripTravelSettings(
  row: Partial<TripTravelSettings> | null | undefined,
): TripTravelSettings {
  return {
    timezone: resolveTripTimezone(row?.timezone),
